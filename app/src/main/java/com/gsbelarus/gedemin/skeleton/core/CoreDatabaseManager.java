...
public class CoreDatabaseManager extends BaseDatabaseManager implements BasicDatabaseOpenHelper.DBOpenHelperCallback {
 ...
 //NUMERIC означает четко указанную точность (общее число цифр), DECIMAL означает как минимум указанную точность
...
//        db.execSQL("CREATE TABLE test ( " + BaseColumns._ID + " INTEGER PRIMARY KEY AUTOINCREMENT, TEST VARCHAR(2) )");
//
//        db.execSQL(Statements.converter().toSql(Statements.insert().
//                into("test")
//                .columns("TEST")
//                .values("1234")), new Object[] {"1234"});
    }
