...
public class CoreDatabaseManager extends BaseDatabaseManager implements BasicDatabaseOpenHelper.DBOpenHelperCallback {
    ...
    private static final String CREATE_TABLE_LOG_CHANGES =
            "CREATE TABLE " + TableLogChanges.TABLE_NAME + " (" +
                    ...
                    TableLogChanges.COLUMN_TABLE_NAME + SQLiteStorageTypes.TEXT + COMMA_SEP +
                    TableLogChanges.COLUMN_INTERNAL_ID + SQLiteStorageTypes.INTEGER + "UNIQUE" + COMMA_SEP +
    ...
    public void notifyVersionDB() {
        db.setVersion(getVersionFromMetadata());
    }

    private int getVersionFromMetadata() {
        int version = 1;
...
//        }
    ...
    public boolean migrateIfNeeded() {
        int versionFromMetadata = getVersionFromMetadata();
        if (db.getVersion() < versionFromMetadata) {
            onUpgradeDatabase(db, db.getVersion(), versionFromMetadata);
    ...
    public Map<String, List<Object>> getDeletedRowsId() {
        Map<String, List<Object>> tables = new HashMap<>();
